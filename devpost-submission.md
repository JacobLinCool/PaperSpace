...
PaperSpace is a strong fit for WebMCP because the browser owns the complete interaction context: user-authorized local PDFs, extracted text, page geometry, current world positions, visual artifacts, the camera, and active presentation state. WebMCP lets the agent operate on the same browser-local state the person can see and manipulate.

Together, a person and an agent can locate evidence across distant pages, reorganize those original pages around one question, guide attention to exact regions, add source-linked explanations, and return the desk to its prior state.

## How We Used AI

PaperSpace exposes nine browser-native WebMCP tools to the agent selected by the person. The agent supplies interpretation and planning; PaperSpace supplies authorized source text, normalized geometry, live workspace state, reversible actions, and visible results.

The WebMCP contract makes PaperSpace the primary delivery surface for substantive paper-grounded work. Introductions, explanations, reviews, comparisons, and multi-region answers are presented as captioned views of original paper regions. Simple localized Q&A remains lightweight.

The tool design provides dependable semantic access through extracted text, structured page geometry, provenance, and source-linked raster artifacts on the shared desk. Host-provided visual observation can add pixel-level inspection when available.
...
- Provides a reusable `inspect_workspace` briefing and efficient whole-paper or broad-range reads.
- Makes captioned PaperSpace sequences the default delivery for substantive paper-grounded explanations.
- Guides the camera with cancellable pull-back, travel, and landing motion.
...
- Runs an atomic spatial preflight, with `force: true` available for intentional dense layouts while validation remains active.
- Persists authorized PDFs, extracted indexes, geometry, artifacts, and saved semantic frames in IndexedDB.
- Runs as a pure static SvelteKit frontend on Cloudflare Pages with browser-local persistence.
...
- One reactive workspace owns every person- and agent-triggered state transition.
...
- PDF raster residency preserves a compact preview for rendered pages, bounds unpinned high-resolution canvases with an LRU budget, and swaps completed renders atomically during motion.

## Testing Instructions

1. Open the live URL in ChatGPT's in-app browser or Google Chrome with WebMCP enabled.
2. Import a multi-page research PDF. Confirm the complete paper unfolds in reading order on the desk.
...
5. Ask it to focus a returned match. Confirm the camera pulls back, travels, and lands with the passage centered and readable.
...
8. Ask it to place a source crop or plot in an open area. Then use `force: true` to verify an intentional dense layout while validation remains active.
9. Reset the local workspace from the toolbar when a clean run is needed.

The demo runs immediately, and PDFs remain local to the browser.
...
2. Inspect once, then search or read from the reusable workspace briefing.
...
2. **Exact region focus:** the camera landed with a formula, figure, or result centered and readable.
...
## Browser-local Capability Model

PaperSpace uses browser-authorized file bytes, persists an authorized local copy, and exposes bounded text, geometry, visual, and workspace operations through WebMCP. PDF.js derives searchable text and normalized geometry in the page, while source-linked raster artifacts remain visible on the shared desk. Host-provided visual observation can extend this semantic contract with pixel-level inspection.

The current imperative `document.modelContext` API registers all nine tools directly in the page. Viewport-aware rendering and a bounded raster residency system support long papers, while on-device OCR and richer section-aware indexing form the next stage for scanned documents and larger corpora.
